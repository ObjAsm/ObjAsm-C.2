-- Title: ADE_UpperCase.lua
-- Purpose: Converts the selection to uppercase.
-- Author: G. Friedrich
...
if hEdit ~= nil then
  ...
  if FirstLine ~= 0 then 
    ...
    do
      ...
      LineText = string.sub(LineText, 1, FirstChar)..string.upper(string.sub(LineText, FirstChar + 1, LastChar + 1))..string.sub(LineText, LastChar + 2, -1)
      Editor.SetLineText(hEdit, i, LineText)
    end
  else
    Application.MsgBox("There is no selection in this editor", "Error", 16)
  end
else
