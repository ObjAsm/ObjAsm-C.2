...
;
;   This implementation focuses on testing the NetCom engine by sending repeated
...
;   logic, while the NetComCoreClient object manages the application lifecycle, engine
;   initialization and status display.
;
...
APP_NAME                    textequ <NetComCoreClient>  ;Application internal name
PROTOCOL_WND_NAME           textequ <Core Client Protocol>  ;Debug/protocol window title
...
.code
include NetComCoreClient_Globals.inc                    ;Application-wide globals
include NetComCoreClient_Main.inc                       ;Main application implementation

start proc                                              ;Program entry point
  SysInit                                               ;Initialize ObjAsm runtime model

  DbgClearTxt "NETCOMCORECLIENT"                        ;Clear main debug output window
  DbgClearTxt "&PROTOCOL_WND_NAME"                      ;Clear protocol debug window
  OCall $ObjTmpl(NetComCoreClient)::NetComCoreClient.Init ;Initialize NetComCoreClient object
  OCall $ObjTmpl(NetComCoreClient)::NetComCoreClient.Run  ;Enter application message loop
  OCall $ObjTmpl(NetComCoreClient)::NetComCoreClient.Done ;Shutdown and cleanup resources
