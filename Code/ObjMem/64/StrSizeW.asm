...
StrSizeW proc pStringW:POINTER
  ...
  lea rax, OR_MASKS_W
  mov r8, [rax + r10*(sizeof(QWORD)/2)]                 ; Preload mask
  mov r11, 0001000100010001h
  mov r10, 8000800080008000h
  or r8, [rdx]                                          ; Suppress pre-string zero CHRWs
...
@@Exit:
  bsf r8, r8                                            ; Return index of 1st set bit (start BIT00)
  shr r8d, 3                                            ; BYTE offset: 0, 2, 4, 6
  lea rax, [rdx + r8 + 1]                               ; End address + 2
  sub rax, rcx                                          ; eax = BYTE count including ZTC
  ret
StrSizeW endp
OPTION PROC:DEFAULT
